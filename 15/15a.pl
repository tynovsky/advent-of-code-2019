...
while (my $path = shift @queue) {
    say scalar(@$path);
    if (go($path, $m, $in, $out, $grid, \$oxygen)) {
        push @queue, (
            [@$path, 1],
            [@$path, 2],
            [@$path, 3],
...
sub read_grid {
    my ($grid, $path) = @_;
    return if !$grid;
    return $grid->{path_to_coords($path)}
}

sub write_grid {
    my ($grid, $path, $symbol) = @_;
    return if !$grid;
    $grid->{path_to_coords($path)} = $symbol
}
...
sub go {
    my ($path, $m, $in, $out, $grid, $oxygen) = @_;

    return if read_grid($grid, $path) // '.' eq '#';
    return if read_grid($grid, $path) // '.' eq '*';
    ...
    for my $step (@$path) {
        ...
        die if @$out > 0;
        if ($o == 0) {
            go(reverse_path(\@prev_steps), $m, $in, $out);
            write_grid($grid, [@prev_steps, $step], '#');
            return
        }
        if ($o == 2) {
            $$oxygen = join(',', (@prev_steps, $step));
            return
        }
        write_grid($grid, [@prev_steps, $step], '*');
        push @prev_steps, $step;
    }
    return 1
}
...
sub reverse_path {
    my ($path) = @_;
    my %rev = ( 1 => 2, 2 => 1, 3 => 4, 4 => 3 );
    return [ map { $rev{$_} } reverse(@$path) ];
}
