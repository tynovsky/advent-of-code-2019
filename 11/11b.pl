...
my $dir = '^';
my $coords = [0, 0];
my $a = Machine->new(<DATA>, $in, $out);

my $iteration = 0;
while () {
    $a->run();
    last if $a->{execution} eq 'halt';
    my $color = shift @$out;
    my $turn = shift @$out;
    $panels{join(',', @$coords)} = $color;
    $dir = turn($turn, $dir);
    go($coords, $dir);
    if (exists $panels{join(',', @$coords)}) {
        push @$in, $panels{join(',', @$coords)};
    }
    else {
        push @$in, 0;
    }
...
sub turn {
    my ($turn, $dir) = @_;
    return turn_left($dir) if $turn == 0;
    return turn_right($dir) if $turn == 1;
    die "wrong turn direction $turn"
}
