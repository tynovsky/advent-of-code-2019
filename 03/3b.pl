...
my @moves1 = split /,/, $line1;
my @moves2 = split /,/, $line2;

my $start = [0, 0];
my $lines1 = lines($start, @moves1);
my $lines2 = lines($start, @moves2);

my @indexes = map { 2 * $_ } 0 .. int(@$lines1 / 2);
my @horizontal1 = @$lines1[@indexes];

@indexes = map { 2 * $_ } 0 .. int(@$lines2 / 2);
my @horizontal2 = @$lines2[@indexes];

@indexes = map { 2 * $_ + 1 } 0 .. int(@$lines1 / 2);
my @vertical1 = @$lines1[@indexes];

@indexes = map { 2 * $_ + 1} 0 .. int(@$lines2 / 2);
my @vertical2 = @$lines2[@indexes];

print Dumper [\@horizontal1, \@horizontal2, \@vertical1, \@vertical2];

my @intersections;
for my $h (@horizontal1) {
	next if !$h;
	for my $v (@vertical2) {
		next if !$v;
		push @intersections, @{intersection($h, $v)}
	}
}
for my $h (@horizontal2) {
	next if !$h;
	for my $v (@vertical1) {
		next if !$v;
		push @intersections, @{intersection($h, $v)}
...
my $min = "inf";
for my $i (@intersections) {
	my $distance = abs($i->[0]) + abs($i->[1]);
...
my $distance = 0;
for my $l (@$lines1) {
	for my $i (get_matching_points($l, @intersections)) {
		$i_distance1{$i->[0] . ',' . $i->[1]} //= $distance + dist($l->[0], $i)
	}
	$distance += dist(@$l[0, 1])
}
...
for my $l (@$lines2) {
	for my $i (get_matching_points($l, @intersections)) {
		$i_distance2{$i->[0] . ',' . $i->[1]} //= $distance + dist($l->[0], $i)
	}
	$distance += dist(@$l[0, 1])
}

print Dumper [\%i_distance1, \%i_distance2];

$min = "inf";
for my $k (keys %i_distance1) {
	my $steps = ($i_distance1{$k} + $i_distance2{$k});
	$min = $steps if $steps < $min && $steps > 0;
}
...
sub intersection {
	...
	if ($horizontal->[0][1] > max($vertical->[0][1], $vertical->[1][1])) {
		return []
	}
	if ($vertical->[0][0] < min($horizontal->[0][0], $horizontal->[1][0])) {
		return []
	}
	if ($vertical->[0][0] > max($horizontal->[0][0], $horizontal->[1][0])) {
		return []
	}
