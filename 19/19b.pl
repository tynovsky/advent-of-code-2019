...
sub test {
    my ($x, $y, $program, $verbose) = @_;
    say "test $x, $y" if $verbose;
    my $out = [];
    my $m = Machine->new($program, [$x, $y], $out);
    $m->run();
    return if ! shift @$out;
    say "\tok x,y" if $verbose;

    $m = Machine->new($program, [$x+99, $y], $out);
    $m->run();
    return if ! shift @$out;
    say "\tok x+99,y" if $verbose;

    $m = Machine->new($program, [$x, $y+99], $out);
    $m->run();
    return if ! shift @$out;
    say "\tok x,y+99" if $verbose;

    $m = Machine->new($program, [$x+99, $y+99], $out);
    $m->run();
    return if ! shift @$out;
    say "\tok x+99,y+99" if $verbose;

    $m = Machine->new($program, [$x+100, $y], $out);
    $m->run();
    return if shift @$out;
    say "\tok x+100,y" if $verbose;

    $m = Machine->new($program, [$x, $y+100], $out);
    $m->run();
    return if shift @$out;
    say "\tok x,y+100" if $verbose;
